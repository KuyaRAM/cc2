...
//1st curl
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://kissme-lingerie.com/cart/');
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');

$headers = array();
$headers[] = 'authority: kissme-lingerie.com';
...
$a = curl_exec($ch);
//VARIATION_ID OR STOCKLEVEL
$variation_id= trim(strip_tags(v($a, '<input type="hidden" name="variation_id" value="','"/>')));

//AAD TO CART

//2nd curl
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://kissme-lingerie.com/shop/accessories/headband/beverly-headband/');
curl_setopt($ch, CURLOPT_POST, 1);

$postfield = '------WebKitFormBoundaryAGF5Q5iivk8hojxx
Content-Disposition: form-data; name="attribute_pa_color"

blue
------WebKitFormBoundaryAGF5Q5iivk8hojxx
...
$headers[] = 'authority: kissme-lingerie.com';
$headers[] = 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7';
$headers[] = 'content-type: multipart/form-data; boundary=----WebKitFormBoundaryAGF5Q5iivk8hojxx';
$headers[] = 'origin: https://kissme-lingerie.com';
$headers[] = 'referer: https://kissme-lingerie.com/shop/accessories/headband/beverly-headband/';
$headers[] = 'user-agent: '.$ua.'';
$headers[] = 'connection: keep-alive';
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $cx, CURLOPT_COOKIEJAR => $cx]);
...
//4th curl
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://kissme-lingerie.com/checkout/');
curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');

$headers = array();
$headers[] = 'authority: kissme-lingerie.com';
$headers[] = 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7';
$headers[] = 'referer: https://kissme-lingerie.com/cart/';
$headers[] = 'user-agent: '.$ua.'';
$headers[] = 'connection: keep-alive';
curl_setopt_array($ch, [CURLOPT_COOKIEFILE => $cx, CURLOPT_COOKIEJAR => $cx]);
curl_setopt_array($ch, array(CURLOPT_HTTPHEADER => $headers, CURLOPT_FOLLOWLOCATION => 1, CURLOPT_RETURNTRANSFER => 1, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYPEER => 0, CURLOPT_SSL_VERIFYHOST => 0));
